...
export async function GET(request, { params }) {
  try {
    ...
    if (kindFilter) {
      const data = await filterAllowedModels(request, await buildModelsList(kindFilter));
    ...
    const models = await filterAllowedModels(request, await buildModelsList([LLM_KIND]));
